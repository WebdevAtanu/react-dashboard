...
function Register() {
	...
	const register=(e)=>{
		...
		sessionStorage.setItem(userMail,JSON.stringify({'name':userName,'email':userMail,'password':userPass}));
		myform.name.value='';
		myform.email.value='';
...
<div className='bg-gray-100 p-5 flex justify-center'>
	<form name='myform' onSubmit={register} className='w-full md:w-1/2 border border-black py-12 px-6 rounded-xl flex flex-col gap-5 bg-white'>
		<label className="input input-bordered flex items-center gap-2">
			<svg
				...
				<input type="text" className="grow" placeholder="Username" name='name' required/>
			</label>
			<label className="input input-bordered flex items-center gap-2">
				<svg
					xmlns="http://www.w3.org/2000/svg"
